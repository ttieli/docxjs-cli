...
function loadPlaywright() {
    if (playwrightLoaded) return chromium;
...
// HTML parser for HTML table support
...
const PAGE_WIDTH_A4_TWIPS = 11906;   // docx 库默认页面（Document 未传 page.size）
...
function parseMarginToTwips(marginStr) {
    ...
    let twips;
    switch (unit) {
        case 'cm':            twips = num * 567; break;
        case 'mm':            twips = num * 56.7; break;
        case 'in':            twips = num * 1440; break;
        case 'pt':            twips = num * 20; break;
        case 'twip':
...
// Constants for math rendering
const MATH_DEVICE_SCALE = 3;  // deviceScaleFactor for high-quality rendering
const MATH_INLINE_FONT_SIZE = 36;  // px - KaTeX base font for inline (match Word 10.5pt text)
const MATH_BLOCK_FONT_SIZE = 32;   // px - KaTeX base font for block (slightly larger than inline)
const MATH_INLINE_MAX_HEIGHT_PX = 60;  // max height in pixels for inline (~2.5x line height)
const MATH_BLOCK_MAX_WIDTH_PX = 550;   // max width in pixels for block (~6 inches at 96dpi)

// Convert rendered pixels (at deviceScaleFactor) to Word pixels
// The transformation expects CSS pixels, so we divide by the scale factor
function renderedToWordPixels(renderedPixels) {
    return renderedPixels / MATH_DEVICE_SCALE;
}
    ...
    try {
        ...
        if (!sharedMathPage || sharedMathPage.isClosed()) {
            const context = await browser.newContext({
                ...
                deviceScaleFactor: MATH_DEVICE_SCALE
            });
        ...
        const fontSize = displayMode ? MATH_BLOCK_FONT_SIZE : MATH_INLINE_FONT_SIZE;
        const renderResult = await sharedMathPage.evaluate(({ code, display, fontSize }) => {
            try {
        ...
        }, { code: mathCode, display: displayMode, fontSize });
        ...
        // Return object with buffer and dimensions for proper scaling
        const result = {
            buffer,
            renderedWidth: renderResult.width,
            renderedHeight: renderResult.height
        };
...
/**
 * Parse HTML table and build a grid with rowspan/colspan support
 * @param {string} htmlContent - HTML table content
 * @returns {object|null} - { rows: [], colCount: number } or null if parsing fails
 */
function parseHtmlTable(htmlContent) {
    try {
        ...
        // First pass: determine grid dimensions and build cell data
        const cellData = [];
        let maxCols = 0;
        ...
        // Create occupancy grid to track merged cells
        const occupied = [];
        for (let r = 0; r < trElements.length + 10; r++) {
            occupied[r] = new Array(maxCols + 10).fill(false);
        }

        // Second pass: build the grid with proper cell placement
        const rows = [];
        trElements.forEach((tr, rowIndex) => {
            ...
            cells.forEach(cell => {
                ...
                // Get cell text content (strip HTML tags but preserve basic formatting)
        ...
        return { rows, colCount: maxCols };
    } catch (err) {
        console.warn(`   ⚠️ HTML table parse error: ${err.message}`);
        return null;
    }
}

/**
 * Convert parsed HTML table to docx Table
 */
async function htmlTableToDocx(md, htmlContent, currentStyle, baseDir, processInlineFn) {
    ...
    // Build occupancy grid for proper cell skipping
    const occupied = [];
    const maxRows = tableData.rows.length;
    for (let r = 0; r < maxRows + 20; r++) {
        occupied[r] = new Array(tableData.colCount + 10).fill(null);
    }

    // Mark occupied cells and store cell references
    tableData.rows.forEach((rowData, rowIndex) => {
        rowData.cells.forEach(cell => {
            for (let r = 0; r < cell.rowSpan; r++) {
                for (let c = 0; c < cell.colSpan; c++) {
                    if (r === 0 && c === 0) {
                        occupied[rowIndex + r][cell.colIndex + c] = { type: 'origin', cell };
                    } else {
                        occupied[rowIndex + r][cell.colIndex + c] = { type: 'spanned' };
                    }
                }
            }
        });
    ...
    // Safety: empty HTML table produces tableData.colCount === 0 — return null
    ...
    for (let rowIndex = 0; rowIndex < tableData.rows.length; rowIndex++) {
        ...
        for (let colIndex = 0; colIndex < tableData.colCount; colIndex++) {
            const occupant = occupied[rowIndex][colIndex];
            ...
            if (!occupant) {
            ...
            } else if (occupant.type === 'spanned') {
                // Skip - this cell is covered by a merge
                continue;
            } else if (occupant.type === 'origin') {
                const cell = occupant.cell;
                const isHeader = cell.isHeader || rowData.isHeader;
                const isBold = isHeader ? tblConfig.headerBold : false;
                const color = isHeader ? tblConfig.headerColor : (currentStyle.colorMain || "000000");
                ...
                const cellTokens = md.parseInline(cell.text, {})[0];
                const cellRuns = await processInlineFn(md, cellTokens, currentStyle, baseDir, color, isBold);
                ...
                // Add merge properties if needed (rowSpan triggers docx auto CONTINUE cell;
                // columnSpan outputs w:gridSpan — both must coexist with explicit width)
                if (cell.rowSpan > 1) {
                ...
                docxCells.push(new TableCell(cellConfig));
            }
        }
...
async function processInline(md, inlineToken, currentStyle, baseDir, colorOverride, boldOverride) {
    ...
    for (const token of inlineToken.children) {
        ...
        // Inline math handling ($...$)
            ...
            try {
                const result = await renderMathLocally(mathCode, false); // displayMode = false for inline
                if (result && result.buffer) {
                    // Convert rendered pixels to Word pixels (divide by deviceScaleFactor)
                    let width = renderedToWordPixels(result.renderedWidth);
                    let height = renderedToWordPixels(result.renderedHeight);

                    // Apply max height limit for inline formulas (1.5x line height)
                    if (height > MATH_INLINE_MAX_HEIGHT_PX) {
                        const ratio = MATH_INLINE_MAX_HEIGHT_PX / height;
                        width = width * ratio;
                        height = MATH_INLINE_MAX_HEIGHT_PX;
                    }
...
async function generateDocx(markdownContent, currentStyle, baseDir, options = {}) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        log(`Token ${i}: ${token.type} (${token.tag})`);
        ...
        if (token.type === 'heading_open') {
            ...
            if (!tableBuffer) {
                ...
                log(`  -> Added Paragraph (blockquote=${blockquoteLevel}, list=${currentListLevel})`);
                i += 2;
            }
        }
        // --- Horizontal Rule (---) ---
            ...
            log(`  -> Added Horizontal Rule`);
        }
        // --- Blockquote ---
        else if (token.type === 'blockquote_open') {
            blockquoteLevel++;
            log(`  -> Blockquote open (level ${blockquoteLevel})`);
        }
        else if (token.type === 'blockquote_close') {
            blockquoteLevel--;
            log(`  -> Blockquote close (level ${blockquoteLevel})`);
        }
        // --- Bullet List (Unordered) ---
        else if (token.type === 'bullet_list_open') {
            listStack.push({ type: 'bullet', level: currentListLevel, counter: 0 });
            currentListLevel++;
            log(`  -> Bullet list open (level ${currentListLevel})`);
        }
        else if (token.type === 'bullet_list_close') {
            listStack.pop();
            currentListLevel--;
            log(`  -> Bullet list close (level ${currentListLevel})`);
        }
        // --- Ordered List ---
            ...
            currentListLevel++;
            log(`  -> Ordered list open (level ${currentListLevel}, start=${start})`);
        }
        else if (token.type === 'ordered_list_close') {
            listStack.pop();
            currentListLevel--;
            log(`  -> Ordered list close (level ${currentListLevel})`);
        }
        // --- List Item ---
        ...
        // --- Math Block ($$...$$) ---
            ...
            log(`  -> Processing Math Block...`);

            try {
                const result = await renderMathLocally(mathCode, true); // displayMode = true
                ...
                if (result && result.buffer) {
                    ...
                    // Apply max width limit for block formulas (page width ~6 inches)
                    if (width > MATH_BLOCK_MAX_WIDTH_PX) {
                        const ratio = MATH_BLOCK_MAX_WIDTH_PX / width;
                        width = MATH_BLOCK_MAX_WIDTH_PX;
                        height = height * ratio;
                    }
                    ...
                    log(`  -> Added Math Block as Image (${Math.round(width)}x${Math.round(height)} px)`);
                } else {
            ...
            if (token.info === 'mermaid') {
                ...
                log(`  -> Processing Mermaid Diagram (Local Only)...`);
                ...
                try {
                    ...
                    if (buffer) {
                        ...
                        // Limit both width and height to fit within a single page
                        const MAX_WIDTH = 550;   // ~6 inches page width
                        const MAX_HEIGHT = 700;  // ~9.7 inches page height (minus margins)

                        // Calculate scale factor to fit both dimensions
                        let scale = 1;
                        if (width > MAX_WIDTH) scale = Math.min(scale, MAX_WIDTH / width);
                        if (height > MAX_HEIGHT) scale = Math.min(scale, MAX_HEIGHT / height);
                        ...
                        log(`  -> Added Mermaid Diagram as Image (${width}x${height} px, scale=${scale.toFixed(2)})`);
                    } else {
            ...
            // Code block - use table container with one Paragraph per line
            const codeText = token.content;
            const codeLines = codeText.split('\n');

            // Remove trailing empty line if exists (common in fenced code blocks)
            if (codeLines.length > 0 && codeLines[codeLines.length - 1].trim() === '') {
                codeLines.pop();
            }

            // Build one Paragraph per line for proper alignment
            const codeParagraphs = codeLines.map(line => {
                // Process indentation: convert tabs to spaces, then all spaces to NBSP
                const processedLine = line
                    .replace(/\t/g, '    ')      // Tab -> 4 spaces
                    .replace(/ /g, '\u00A0');    // Space -> NBSP (non-breaking space)

                return new Paragraph({
                    children: [new TextRun({
                        text: processedLine || '\u00A0',  // NBSP for empty lines
                        font: "Consolas",
                        size: 18,  // 9pt
                        color: "2B2B2B"
                    })],
                    spacing: {
                        line: 240,      // Single line spacing (240 twips = 1x)
                        before: 0,
                        after: 0
                    }
                });
            ...
            // Add spacing paragraph before code block (use table spacing config)
            const codeBlockConfig = currentStyle.table || {};
            const codeSpacingBefore = codeBlockConfig.spacingBefore !== undefined ? codeBlockConfig.spacingBefore : 40;
            if (codeSpacingBefore > 0) {
                docChildren.push(new Paragraph({
                    children: [],
                    spacing: { before: 0, after: codeSpacingBefore }
                }));
            }
            docChildren.push(codeTable);
            // Add spacing paragraph after code block
            const codeSpacingAfter = codeBlockConfig.spacingAfter !== undefined ? codeBlockConfig.spacingAfter : 0;
            if (codeSpacingAfter > 0) {
                docChildren.push(new Paragraph({
                    children: [],
                    spacing: { before: codeSpacingAfter, after: 0 }
                }));
            }
            log(`  -> Added Code Block (${codeLines.length} lines)`);
        }
        // --- HTML Block (check for HTML tables) ---
        else if (token.type === 'html_block') {
            const htmlContent = token.content.trim();
            // Check if this is an HTML table
            if (htmlContent.toLowerCase().startsWith('<table')) {
                log(`  -> Processing HTML Table with rowspan/colspan...`);
                try {
                    const table = await htmlTableToDocx(md, htmlContent, currentStyle, baseDir, processInline);
                    if (table) {
                        // Add spacing paragraph before table (configurable)
                        const htmlTblConfig = currentStyle.table || {};
                        const htmlTableSpacingBefore = htmlTblConfig.spacingBefore !== undefined ? htmlTblConfig.spacingBefore : 40;
                        if (htmlTableSpacingBefore > 0) {
                            docChildren.push(new Paragraph({
                                children: [],
                                spacing: { before: 0, after: htmlTableSpacingBefore }
                            }));
                        }
                        docChildren.push(table);
                        // Add spacing paragraph after table (configurable)
                        const htmlTableSpacingAfter = htmlTblConfig.spacingAfter !== undefined ? htmlTblConfig.spacingAfter : 0;
                        if (htmlTableSpacingAfter > 0) {
                            docChildren.push(new Paragraph({
                                children: [],
                                spacing: { before: htmlTableSpacingAfter, after: 0 }
                            }));
                        }
                        log(`  -> Added HTML Table`);
                    } else {
                        // Fallback: show as text if parsing failed
                        docChildren.push(new Paragraph({
                            children: [new TextRun({
                                text: `[HTML Table Parse Failed]`,
                                color: "FF0000"
                            })]
                        }));
                    }
                } catch (err) {
                    console.warn(`   ⚠️ HTML table processing failed: ${err.message}`);
                    docChildren.push(new Paragraph({
                        children: [new TextRun({
                            text: `[HTML Table Error: ${err.message}]`,
                            color: "FF0000"
                        })]
                    }));
                }
            } else {
                // Non-table HTML block - render as plain text or skip
                // You could add more HTML handling here if needed
                log(`  -> Skipping non-table HTML block`);
            }
        }
            ...
            if (tableBuffer && tableBuffer.rows.length > 0) {
                ...
                // Add spacing paragraph before table (configurable)
                const tableSpacingBefore = tblConfig.spacingBefore !== undefined ? tblConfig.spacingBefore : 40;
                if (tableSpacingBefore > 0) {
                    docChildren.push(new Paragraph({
                        children: [],
                        spacing: { before: 0, after: tableSpacingBefore }
                    }));
                }
                ...
                // Add spacing paragraph after table (configurable)
                const tableSpacingAfter = tblConfig.spacingAfter !== undefined ? tblConfig.spacingAfter : 0;
                if (tableSpacingAfter > 0) {
                    docChildren.push(new Paragraph({
                        children: [],
                        spacing: { before: tableSpacingAfter, after: 0 }
                    }));
                }
